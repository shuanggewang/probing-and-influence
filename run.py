...
if __name__ == '__main__':
    ...
    while not gameExit:
        ...
        if timer == 0:
            ...
            y2.append(controls[5]*config.d_t)
            y3.append(controls[6]*config.d_t)
        else:
            x.append(timer/10)
            y1.append(y1[-1]+controls[1]*config.d_t)
            y2.append(y2[-1]+controls[5]*config.d_t)
            y3.append(y3[-1]+controls[6]*config.d_t)
        ...
        if (min(state.v)>=19 and timer >= 400):
            figure.savefig("./figures/deviation2.jpg", dpi=300)
            input()
